(() => {
  ...
  async function connect(event) {
    ...
    try {
      ...
      setMessage($('connectMessage'), data.capabilities.zoneRead ? 'Token verified and zones loaded.' : 'Token verified. Zone listing is unavailable; use a manual Zone ID if needed.', 'success');
  ...
  function setMode(mode) {
    ...
    $('queryRule').textContent = isLogpull ? 'Logpull: maximum 1 hour; end time must be at least 5 minutes in the past.' : 'Security Events availability depends on your plan and Analytics Read access.';
  ...
  async function runQuery() {
    ...
    const start = isoFromLocalInput($('startTime').value), end = isoFromLocalInput($('endTime').value);
    if (!zoneId) return setMessage(queryMessage, 'Select a zone first.');
    if (!start || !end) return setMessage(queryMessage, 'Select a valid time range.');
  ...
  function renderResults(range) {
    ...
    $('resultMeta').textContent = `${state.resultSource === 'security-events' ? 'Security Events' : 'HTTP Logpull'} · ${range.start || ''} → ${range.end || ''}`;
    $('resultSearch').value = '';
  ...
  function renderTable(rows) {
    ...
    rows.slice(0, 2000).forEach(r => { const tr = document.createElement('tr'); cols.forEach(c => { const td = document.createElement('td'); td.textContent = cellValue(r[c]); td.title = cellValue(r[c]); tr.appendChild(td); }); body.appendChild(tr); });
    $('visibleRows').textContent = `${Math.min(rows.length,2000).toLocaleString()} displayed${rows.length>2000?' (table capped at 2,000)':''}`;
  }
  ...
  function fileBase() { const zone = state.zones.find(z=>z.id===zoneSelect.value); const name = (zone?.name || zoneSelect.value || 'zone').replace(/[^a-z0-9.-]+/gi,'-'); return `cloudflare-${state.resultSource || state.mode}-${name}-${new Date().toISOString().slice(0,19).replace(/[:T]/g,'-')}`; }
